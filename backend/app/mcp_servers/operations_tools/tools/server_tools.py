...
SERVER_TOOLS: List[Tool] = [
    Tool(
        ...
        inputSchema={
            ...
            "properties": {
                ...
                "status": {"type": "boolean", "description": "状态过滤（True启用/False禁用）"},
                "ip": {"type": "string", "description": "IP地址过滤"},
    ...
    Tool(
        ...
        inputSchema={
            "type": "object",
            "properties": {"id": {"type": "integer", "description": "服务器节点ID"}},
            "required": ["id"]
        }
    ),
    Tool(
        ...
        inputSchema={
            "type": "object",
            "properties": {
                "ip": {"type": "string", "description": "节点IP地址"},
                "port": {"type": "integer", "description": "SSH端口", "default": 22},
                "project": {"type": "string", "description": "所属项目"},
                "service_id": {"type": "integer", "description": "主服务模块ID"},
                "service_ids": {"type": "array", "items": {"type": "integer"}, "description": "关联的服务模块ID列表"},
                "status": {"type": "boolean", "description": "是否启用", "default": True},
                "description": {"type": "string", "description": "备注说明"},
                "idc": {"type": "string", "description": "机房"},
                "tags": {"type": "string", "description": "服务器标签"},
                "operator_type": {"type": "string", "description": "操作类型(deploy:部署, restart:重启, init:初始化 等)"}
            },
            "required": ["ip"]
        }
    ),
    Tool(
        ...
        inputSchema={
            "type": "object",
            "properties": {
                "id": {"type": "integer", "description": "服务器节点ID"},
                "ip": {"type": "string", "description": "节点IP地址"},
                ...
                "idc": {"type": "string", "description": "机房"},
                "tags": {"type": "string", "description": "服务器标签"},
                "operator_type": {"type": "string", "description": "操作类型"}
            },
            "required": ["id"]
        }
    ),
    Tool(
        ...
        inputSchema={
            "type": "object",
            "properties": {
                "ids": {"type": "array", "items": {"type": "integer"}, "description": "要删除的节点ID列表"}
            },
            "required": ["ids"]
        }
    )
...
async def handle_server_tool(name: str, arguments: Dict[str, Any], auth: AuthSchema) -> List[TextContent]:
    """处理服务器管理工具调用"""
    ...
    try:
        if name == "list_servers":
            ...
            await ServerService.delete_server_service(
                auth=auth,
                ids=arguments["ids"]
            )
            
            logger.info(f"[MCP工具返回] delete_servers - 删除完成")
