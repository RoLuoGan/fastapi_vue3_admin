...
TASK_TOOLS: List[Tool] = [
                ...
                "operator_metas": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "service_id": {"type": "integer", "description": "服务模块ID"},
    ...
    Tool(
        ...
        inputSchema={
            "type": "object",
            "properties": {"task_id": {"type": "integer", "description": "任务ID"}},
            "required": ["task_id"]
        }
    ),
    Tool(
...
async def handle_task_tool(name: str, arguments: Dict[str, Any], auth: AuthSchema) -> List[TextContent]:
    """处理任务管理工具调用"""
    ...
    if name == "execute_task":
        result = await TaskService.execute_task_service(
            ...
            operator_metas=arguments["operator_metas"]
        )
        return [TextContent(
            type="text",
            text=f"任务已创建并开始执行\n任务ID: {result.get('task_id')}\n任务类型: {result.get('task_type')}\n操作类型: {result.get('operator_type')}"
        )]
        ...
        result = await TaskService.get_task_log_service(
            auth=auth,
            task_id=arguments["task_id"]
        )
        return [TextContent(
